...
export const FlatsDetails = () => {
  ...
  const getResidents = async () => {
    await axios.get(`http://localhost:8000/residents/all/${id}`).then((res) => {
  ...
  const getdetails = async () => {
    await axios
      .get(`http://localhost:8000/flats/1/${id}`)
      .then((res) => {
        setDetails(res.data);
      })
      .catch((er) => {
  ...
  const deleteResident = async (resid) => {
    await axios
      .delete(`http://localhost:8000/residents/delete/${resid}`)
      .then(async (res) => {
        await axios
          .patch(`http://localhost:8000/flats/edit/delete/${id}`)
          .then((res) => {
            getdetails();
            getResidents();
          });
